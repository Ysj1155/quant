...
5. **회고 로그**: 이번주 수동 메모를 먼저 쓰고, 자동 요약과 빠진 주를 보조로 확인합니다.
6. **회고 리포트**: 선택 기간 리포트를 저장하고 기존 마크다운 회고를 읽고 편집합니다.
7. **예측 실험**: 계좌/환율 예측값과 실제값의 오차만 낮은 우선순위의 참고 자료로 확인합니다.
...
- 투자 이벤트 타임라인과 매수/매도 맥락을 분리한 사후검증 데이터셋
- 이번주 회고 로그 작업대, 수동 메모, 태그, 빠진 주 확인
- 선택 기간 회고 리포트 저장/미리보기/편집
- 날짜별 스냅샷 상세와 낮은 우선순위의 예측 오차 실험
...
예측 기능은 투자 결정을 대신하지 않습니다. 이 프로젝트에서 예측은 핵심 기능이 아니라, 실제 계좌 흐름과 단순 모델의 오차를 저장해 나중에 복기하기 위한 낮은 우선순위의 보조 실험입니다.

- 계좌 총액 예측 실험값 저장
- USD/KRW 환율 예측 실험값 저장
- Linear, Holt, ARIMA 모델 지원
- 모델별 MAE/RMSE/MAPE/방향성 정확도 계산과 오차 이력 확인
- 예측 스냅샷 저장과 실제값 반영 구조

## 회고 로그와 리포트

- `회고 로그` 탭은 최신 스냅샷 기준 이번주 내 생각을 먼저 쓰는 작업대입니다.
- 수동 메모와 태그를 상단에 두고, 자동 요약과 다음 질문은 접힌 보조 근거로 확인합니다.
- 최근 주간 로그 중 빠진 주를 보여주고, 회고 루틴이 끊긴 구간을 다시 찾기 쉽게 합니다.
...
- 생성된 자동 요약은 접힌 근거 자료이고, 최종 해석은 수동 회고 메모에 남깁니다.
- 이벤트 사후검증은 성적표가 아니라 복기 질문지로 봅니다.
- 매수는 진입 근거 검증, 매도는 방어 효과와 기회비용 점검으로 분리합니다.
- 7/30관측치 결과 라벨은 `risk_bucket` 기준으로 다르게 해석합니다. 현재 기준은 고변동 ±10%, 중간 ±5%, 낮음 ±3%입니다.
- 현금 비중과 동일 자산 ID 안정화는 당장 핵심 범위에서 제외합니다.
